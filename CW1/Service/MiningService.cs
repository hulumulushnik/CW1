...
namespace BlockChainP411NEW.Services
{
    public class MiningService
    {
        ...
        public void MineBlock(Block block, int difficulty)
        {
            ...
            // Index, TimeStamp, Merkle root (транзакції) та PreviousHash не залежать від Nonce,
            // тому рахуємо їх ОДИН РАЗ до циклу, а не на кожній спробі nonce.
            // Раніше ComputeHash(block) перебудовував весь Merkle-дерево (і друкував Level 0/1/...)
            // на кожній ітерації циклу — це і спричиняло нескінченний спам у консолі та величезне сповільнення майнінгу.
            string baseData = _hashingService.GetBaseBlockData(block);

            do
            {
                block.Nonce++;
                block.Hash = _hashingService.ComputeHash(baseData + $"{block.Nonce}{block.Difficulty}");
            }
